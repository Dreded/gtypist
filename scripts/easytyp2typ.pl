...
my $lines_per_drill = 11; # 1-11 for [Dd]: and D:, 1-22 for [Ss]:
...
sub convert_lesson($**)
{
    ...
    while (defined($line = <$ktouchfile>) && !isBlank($line))
    {
	chomp($line);
if (isComment($line)) {
    $drill_name = $line;
    $drill_name =~ s/^[ \t]*\#//;
    $line_counter = -1; ++$drill_counter;
  }
	if ($line_counter == 0) {

	    print $typfile "*:LESSON${lesson_counter}_D$drill_counter\n";
	    print $typfile "I:($drill_counter)$drill_name\n";
	    print $typfile "${drill_type}$line\n";
      print "\tDrill $drill_counter: $drill_name\n";
...
{
    if (substr($ktouchfilename, rindex($ktouchfilename, ".")) ne ".etyp" ||
	! (-f $ktouchfilename)) { 
    ...
    print TYPFILE "# created by easytyp2typ.pl from " . 
